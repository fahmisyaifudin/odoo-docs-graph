...
def generate_direct_llm(
    ...
    system_prompt = """You are an expert ERP consultant specializing in Odoo version 17. Your task is to answer user questions about Odoo features based on your knowledge.
...
def generate_llm_document_reasoning(
    question: str,
    graph_context: str,
    seed_results: List[Dict[str, Any]],
    client: OpenAI,
    ...
    """
    Use LLM to generate reasoning based on document context results.
    ...
    # Build the system prompt
    system_prompt = """You are an expert ERP consultant. Your task is to answer user questions about Odoo features based on the provided knowledge graph context.

## Your Capabilities:
1. Analyze the document context
2. Identify relevant features, modules, and configurations
3. Provide accurate, specific answers based on the document context
4. Explain your reasoning clearly
...
You must respond in this JSON structure:
{
    ...
    "related_features": ["List of related features from the document"],
    "limitations": "Any limitations or caveats to the answer, or 'None'"
}
...
## Task:
Based on the document context provided above, answer the user's question.
Provide your response in the required JSON format.
"""
